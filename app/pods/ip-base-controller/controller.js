...
export default Controller.extend({
    ...
    refreshGraph: function(ip) {
        ...
        let self = this;
        this.filter.ip = ip;
        self.get('store').queryRecord(this.graphModelName, {
            filter: this.filter
        }).then(function(data) {
    ...
    fetchMySites: function(onFetchDone) {
        var self = this;
        self.get('store').query("userWebsite", {
            filter: {}
        }).then(function(data) {
    ...
    showAlert: function(type, message, dontWait) {
        let arg = {};
        if (typeof dontWait == 'undefined') arg = {
            autoClear: true
        };
        arg.htmlContent = true;
        switch (type) {
            case "info":
                return this.get("notifications").info(message, arg);
                break;
            case "success":
                return this.get("notifications").success(message, arg);
                break;
            case "error":
                return this.get("notifications").error(message, arg);
                break;
            case "warning":
                return this.get("notifications").warning(message, arg);
                break;
        }
    },
